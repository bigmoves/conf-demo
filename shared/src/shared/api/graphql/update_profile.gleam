...
pub fn update_profile_response_to_json(input: UpdateProfileResponse) -> json.Json {
  json.object(
    [
      #("updateOrgAtmosphereconfProfile", org_atmosphereconf_profile_to_json(
        input.update_org_atmosphereconf_profile,
      )),
    ],
  )
}

pub fn update_profile(client: squall.Client, rkey: String, input: OrgAtmosphereconfProfileInput) {
